...
describe('getDynamicHomeNodeNavigation', () => {
    it('returns navigation metadata for router targets and external urls', () => {
        expect(getDynamicHomeNodeNavigation(menuNodes[0], '/knowage-vue')).toEqual({
            type: 'to',
            value: '/document-browser',
            href: '/knowage-vue/document-browser'
        })
...
describe('renderDynamicHomeSrcdoc', () => {
    it('injects href and labels for selected menu placeholders', () => {
        ...
        expect(links[0].getAttribute('data-kn-menu-navigation')).toBe('/document-browser')
        expect(links[0].getAttribute('data-kn-menu-navigation-type')).toBe('to')
    ...
    it('adds navigation metadata to non-anchor placeholders without forcing href', () => {
        const template: IDynamicHomeTemplate = {
            html: '<div><button class="tile" data-kn-menu><span data-kn-label>Menu Item</span></button></div>',
            css: '',
            menuPlaceholders: [{ index: 0, menuIds: [1] }]
        }
        ...
        expect(button?.getAttribute('data-kn-menu-navigation-type')).toBe('to')
