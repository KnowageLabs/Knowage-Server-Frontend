    ...
    <q-dialog :model-value="visibility" persistent @hide="closeDialog">
        <q-card class="exportDialog">
            ...
            <q-card-section>
                ...
                <q-checkbox v-model="exportPersonalFolder" class="q-mt-sm" :label="$t('importExport.users.includePersonalFolder')" style="margin-left: -10px" />
            </q-card-section>
...
export default defineComponent({
    name: 'export-users-dialog',
    props: {
        visibility: Boolean
    },
    emits: ['update:visibility', 'export'],
    data() {
        return { fileName: '', exportPersonalFolder: true }
    },
    methods: {
        closeDialog(): void {
        ...
        emitExport(): void {
            this.$emit('export', this.fileName, this.exportPersonalFolder)
        }
    }
})
</script>
