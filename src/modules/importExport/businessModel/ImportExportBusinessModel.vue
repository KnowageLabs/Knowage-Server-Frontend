<template>
    <div class="import-export-business-models">
        ...
        <q-card class="p-d-flex p-flex-column kn-flex kn-overflow">
            <q-table class="sticky-header-table" ref="businessModelsTable" v-model:selected="selectedItems[FUNCTIONALITY]" :rows="filteredBusinessModels" :columns="columns" row-key="id" selection="multiple" :visible-columns="visibleColumns" virtual-scroll :pagination.sync="pagination" :rows-per-page-options="[0]" flat dense />
        </q-card>
    </div>
</template>
