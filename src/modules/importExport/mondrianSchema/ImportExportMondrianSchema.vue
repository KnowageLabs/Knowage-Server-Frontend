<template>
    <div class="import-export-mondrian-schemas">
        ...
        <q-card class="p-d-flex p-flex-column kn-flex kn-overflow">
            <q-table class="sticky-header-table" ref="mondrianSchemasTable" v-model:selected="selectedItems[FUNCTIONALITY]" :rows="filteredMondrianSchemas" :columns="columns" row-key="id" selection="multiple" :visible-columns="visibleColumns" virtual-scroll :pagination.sync="pagination" :rows-per-page-options="[0]" flat dense />
        </q-card>
    </div>
</template>
