<template>
    <Dialog :header="$t('managers.glossary.common.word', 1)" :breakpoints="glossaryDefinitionDialogDescriptor.dialog.breakpoints" :style="glossaryDefinitionDialogDescriptor.dialog.style" :visible="visible" :modal="true" :closable="false" class="p-fluid kn-dialog--toolbar--primary">
        <div class="p-mt-3">
            <form class="p-fluid p-formgrid p-grid">
                <div class="p-field p-col-4 p-mb-3">
                    ...
                    <KnValidationMessages class="p-mt-1" :v-comp="v$.word.WORD" :additional-translate-params="{ fieldName: $t('managers.glossary.common.word', 1) }"></KnValidationMessages>
                </div>
...
export default defineComponent({
    ...
    methods: {
        ...
        async loadWords(word: string) {
            this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/1.0/glossary/listWords?WORD=` + word).then((response: AxiosResponse<any>) => (this.filteredWords = response.data))
        },
        searchWord(event) {
        ...
        async saveContent(wordId: number) {
            await this.$http
                .post(import.meta.env.VITE_KNOWAGE_CONTEXT + '/restful-services/1.0/glossary/business/addContents', { GLOSSARY_ID: this.selectedGlossaryId, PARENT_ID: this.word.PARENT.CONTENT_ID, WORD_ID: wordId })
                .then((response: AxiosResponse<any>) => {
                    if (response.data.Status !== 'NON OK') {
