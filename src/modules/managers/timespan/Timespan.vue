...
export default defineComponent({
    ...
    methods: {
        async loadTimespans() {
            this.loading = true
            await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/1.0/timespan/listDynTimespan`).then(
        ...
        async loadCategories() {
            this.loading = true
            await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/domains/listValueDescriptionByType?DOMAIN_TYPE=TIMESPAN_CATEGORY`).then((response: AxiosResponse<any>) => (this.categories = response.data))
            this.loading = false
        },
        ...
        async deleteTimespan(id: number) {
            this.loading = true
            await this.$http
                .post(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/1.0/timespan/deleteTimespan?ID=${id}`)
