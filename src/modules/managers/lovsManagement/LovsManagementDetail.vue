...
export default defineComponent({
    ...
    methods: {
        ...
        async loadProfileAttributes() {
            await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/2.0/attributes/`).then((response: AxiosResponse<any>) => (this.profileAttributes = response.data))
        },
        decode() {
        ...
        async formatLov() {
            if (lovProviderEnum.SCRIPT in this.selectedLov.lovProviderJSON) {
                ...
                await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/1.0/datasets/dataset/id/${this.selectedLov.lovProviderJSON.DATASET.ID}`).then((response: AxiosResponse<any>) => {
