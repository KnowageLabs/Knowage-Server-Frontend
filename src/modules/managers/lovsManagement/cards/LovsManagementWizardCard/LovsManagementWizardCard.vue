<template>
  <q-card class="p-m-2">
    <q-toolbar class="kn-toolbar kn-toolbar--secondary">
      ...
      >
        <q-tooltip>{{ $t('managers.lovsManagement.preview') }}</q-tooltip>
      </q-btn>
      <q-btn
          flat
          round
      ...
      >
        <q-tooltip>{{ $t('managers.lovsManagement.test') }}</q-tooltip>
      </q-btn>
      <q-btn
    ...
    <q-card-section>
      <LovsManagementQuery v-if="lovType === 'QUERY'" :selected-lov="lov" :selected-query="selectedQuery"
                           :datasources="datasources" :code-input="codeInput"
                           @touched="onTouched"></LovsManagementQuery>
      <LovsManagementScript v-else-if="lovType === 'SCRIPT'" :selected-lov="lov" :selected-script="selectedScript"
                            :list-of-script-types="listOfScriptTypes" @touched="onTouched"></LovsManagementScript>
      <LovsManagementFixedLovsTable v-else-if="lovType === 'FIX_LOV'" :list-for-fix-lov="listForFixLov"
                                    @touched="$emit('touched')"
                                    @sorted="$emit('sorted', $event)"></LovsManagementFixedLovsTable>
      <LovsManagementJavaClassInput v-else-if="lovType === 'JAVA_CLASS'" :selected-java-class="selectedJavaClass"
                                    @touched="onTouched"></LovsManagementJavaClassInput>
      <LovsManagementDataset v-else-if="lovType === 'DATASET'" :dataset="selectedDataset"
                             @selected="$emit('selectedDataset', $event)"/>
    </q-card-section>
  </q-card>
  <LovsManagementInfoDialog v-show="infoDialogVisible" :visible="infoDialogVisible" :info-title="infoTitle"
                            :lov-type="lov.itypeCd" @close="infoDialogVisible = false"></LovsManagementInfoDialog>
  <LovsManagementProfileAttributesList v-show="profileAttributesDialogVisible" :visible="profileAttributesDialogVisible"
                                       :profile-attributes="profileAttributes" @selected="setCodeInput($event)"
                                       @close="profileAttributesDialogVisible = false"></LovsManagementProfileAttributesList>
  <LovsManagementParamsDialog v-show="paramsDialogVisible" :visible="paramsDialogVisible"
                              :dependencies-list="dependenciesList" :mode="paramsDialogMode" @preview="onPreview"
                              @close="onParamsDialogClose" @test="onTest"></LovsManagementParamsDialog>
  <LovsManagementPreviewDialog v-show="previewDialogVisible" :visible="previewDialogVisible"
                               :data-for-preview="dataForPreview" :pagination="pagination" @close="onPreviewClose"
                               @pageChanged="previewLov($event, false, true)"></LovsManagementPreviewDialog>
...
import LovsManagementProfileAttributesList
  from './LovsManagementProfileAttributesList/LovsManagementProfileAttributesList.vue'
  ...
  methods: {
    ...
    setTreeLovModel() {
      ...
      for (let i = 0; i < this.formatedValues.length; i++) {
        this.testLovTreeModel.push({
          level: this.formatedValues[i],
          value: this.formatedValues[i],
          description: this.formatedDescriptionValues[i]
        })
      }
    },
