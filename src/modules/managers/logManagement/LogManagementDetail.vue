    ...
    <div class="kn-page">
        ...
        <div class="kn-page-content">
            <q-toolbar class="kn-toolbar kn-toolbar--default">
                <q-toolbar-title>{{ $t('managers.logManagement.selectedFiles', { num: selectedFiles.length, count: selectedFiles.length }) }}</q-toolbar-title>
            ...
            <LogManagementImportFileDialog v-model:visibility="importFile" :path="folder.key" :existing-files="files" @fileUploaded="fileUploaded" />
...
import { IFileTemplate } from '@/modules/managers/logManagement/LogManagement'
...
import LogManagementImportFileDialog from './LogManagementImportFileDialog.vue'
import { downloadDirectFromResponse } from '@/helpers/commons/fileHelper'
import mainStore from '../../../App.store'
import { mapActions } from 'pinia'

export default defineComponent({
    components: { Breadcrumb, LogManagementImportFileDialog },
    beforeRouteUpdate() {
    ...
    data() {
        return {
            ...
            columns: [
                {
                    field: 'name',
                    label: this.$t('managers.logManagement.column.name'),
                    name: 'name',
                    sortable: true,
                    align: 'left'
                },
                {
                    field: 'size',
                    label: this.$t('managers.logManagement.column.size'),
                    name: 'size',
                    sortable: true,
                    align: 'right'
                },
                {
                    field: 'lastModified',
                    label: this.$t('managers.logManagement.column.lastModified'),
                    name: 'lastModified',
                    sortable: true,
                    align: 'right'
                }
    ...
    methods: {
        ...
        downloadFiles() {
            this.$http
                .post(import.meta.env.VITE_KNOWAGE_API_CONTEXT + `/api/2.0/logs/files/download`, this.getKeyAndFilenamesObj(), {
        ...
        async loadSelectedFolder() {
            ...
            if (this.folder) {
                await this.$http
                    .get(import.meta.env.VITE_KNOWAGE_API_CONTEXT + `/api/2.0/logs/files` + '?key=' + this.folder.key)
                    .then((response: AxiosResponse<any>) => {
        ...
        deleteFiles() {
            ...
            this.$http
                .delete(import.meta.env.VITE_KNOWAGE_API_CONTEXT + `/api/2.0/logs/files`, {
                    headers: {
                        'Content-Type': 'application/json'
                    },
