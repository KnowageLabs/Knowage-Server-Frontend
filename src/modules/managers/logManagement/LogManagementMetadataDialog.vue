<template>
    <div class="managerDetail">
        <Dialog class="kn-dialog--toolbar--primary knMetadataDialog" :visible="visibility" footer="footer" :header="$t('managers.logManagement.metadata.dialog.title')" :closable="false" modal @id="loadMetadata">
...
import { iModelMetadataTemplate } from './LogManagement'
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            checked: false,
            descriptor: logManagementDescriptor,
    ...
    methods: {
        ...
        loadMetadata() {
            ...
            if (this.id) {
                this.$http
                    .get(import.meta.env.VITE_KNOWAGE_API_CONTEXT + `/api/2.0/logs/files/metadata?key=` + this.id)
        ...
        saveMetadata(): void {
            ...
            if (this.id) {
                this.$http
                    .post(import.meta.env.VITE_KNOWAGE_API_CONTEXT + `/api/2.0/logs/files/metadata?key=` + this.id, this.metadata, {
