<template>
  <div>
    <div v-if="document.engine != 'knowagetalendengine'" id="top-container" class="row q-col-gutter-sm q-ma-md">
      <div class="col-lg-6 col-md-12">
        <q-checkbox v-model="document.saveassnapshot" class="ellipsis-2-lines"
                    :label="$t('managers.scheduler.saveAsSnapshot')" data-test="snapshot-checkbox"/>
        <q-checkbox v-model="document.saveasfile" class="ellipsis-2-lines" :label="$t('managers.scheduler.saveAsFile')"
                    data-test="file-checkbox"/>
        <q-checkbox v-model="document.saveasdocument" class="ellipsis-2-lines"
                    :label="$t('managers.scheduler.saveAsDocument')" data-test="document-checkbox"/>
        <q-checkbox v-model="document.sendmail" class="ellipsis-2-lines" :label="$t('managers.scheduler.sendMail')"
                    data-test="mail-checkbox"/>
      </div>
      <q-select class="col-lg-6 col-md-12 col-sm-12" filled dense v-model="document.outputType"
                :options="descriptor.documentOutputTypes" emit-value
                map-options option-value="value" option-label="label"
                :label="$t('managers.widgetGallery.outputType')"/>
    </div>
    ...
    <div v-if="document && document.engine != 'knowagetalendengine'" class="q-pa-sm">
      <SchedulerSnapshotAccordion v-if="document.saveassnapshot" class="q-ma-sm" :propDocument="document"
                                  data-test="snapshot-accordion"></SchedulerSnapshotAccordion>
      <SchedulerFileAccordion v-if="document.saveasfile" class="q-ma-sm" :propDocument="document"
                              data-test="file-accordion"></SchedulerFileAccordion>
      <SchedulerDocumentAccordion v-if="document.saveasdocument" class="q-ma-sm" :propDocument="document"
                                  :functionalities="functionalities" :datasets="datasets" :jobInfo="jobInfo"
                                  data-test="document-accordion"></SchedulerDocumentAccordion>
...
export default defineComponent({
  ...
  components: {
    SchedulerSnapshotAccordion,
    SchedulerFileAccordion,
    SchedulerJavaClassAccordion,
    SchedulerDocumentAccordion,
    SchedulerMailAccordion,
  },
  props: {
    ...
    datasets: {type: Array},
    jobInfo: {type: Object},
    documentWithUniqueMail: {type: Object}
  },
