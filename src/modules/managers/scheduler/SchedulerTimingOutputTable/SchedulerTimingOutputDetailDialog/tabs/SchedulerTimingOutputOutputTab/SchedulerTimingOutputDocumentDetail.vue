<template>
    <div>
        <div v-if="document.engine != 'knowagetalendengine'" id="top-container" class="row q-col-gutter-sm q-ma-md">
            ...
            <q-select class="col-lg-6 col-md-12 col-sm-12" filled dense v-model="tempOutputType" :options="descriptor.documentOutputTypes" emit-value map-options option-value="value" option-label="label" multiple :label="$t('managers.widgetGallery.outputType')" @update:model-value="changeOutputType" />
        </div>
        ...
        <div v-if="document && document.engine != 'knowagetalendengine'" class="q-pa-sm">
            ...
            <SchedulerMailAccordion v-if="document.sendmail" class="q-ma-sm" :propDocument="document" :datasets="datasets" :jobInfo="jobInfo" :documentWithUniqueMail="documentWithUniqueMail" @sendUniqueMailSelected="setSetUniqueMailSelected" @uniqueMailOptionsChanged="$emit('uniqueMailOptionsChanged', $event)" data-test="mail-accordion"></SchedulerMailAccordion>
        </div>
    </div>
</template>
...
export default defineComponent({
    ...
    methods: {
        ...
        changeOutputType() {
            this.document.outputType = this.tempOutputType.join(',')
        },
        formatDocument() {
            if (!this.document.outputType) this.document.outputType = 'HTML'
            else this.tempOutputType = this.document.outputType.split(',')
