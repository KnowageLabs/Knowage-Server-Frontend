...
export default defineComponent({
    ...
    methods: {
        ...
        async deletePhysicalTable(physicalTable: any) {
            ...
            await this.$http
                .post(import.meta.env.VITE_KNOWAGEMETA_CONTEXT + `/restful-services/1.0/metaWeb/deletePhysicalColumnfromBusinessView`, postData)
        ...
        async addNewPhysicalTables(selectedTables: any[]) {
            ...
            await this.$http
                .post(import.meta.env.VITE_KNOWAGEMETA_CONTEXT + `/restful-services/1.0/metaWeb/addPhysicalColumnToBusinessView`, postData)
                .then((response: AxiosResponse<any>) => {
                    this.meta = applyPatch(this.meta, response.data).newDocument
                    this.loadData()
