...
export default defineComponent({
    ...
    methods: {
        async loadAllSchedules() {
            this.loading = true
            await this.$http
                .get(import.meta.env.VITE_KNOWAGE_CONTEXT + '/restful-services/1.0/kpi/listSchedulerKPI')
        ...
        startSchedule(schedule: iKpiSchedule) {
            ...
            this.$http.post(import.meta.env.VITE_KNOWAGE_CONTEXT + '/restful-services/scheduler/' + action + query).then((response: AxiosResponse<any>) => {
        ...
        async deleteSchedule(scheduleId: number) {
            await this.$http.delete(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/1.0/kpi/${scheduleId}/deleteKpiScheduler`).then(() => {
                this.store.setInfo({
