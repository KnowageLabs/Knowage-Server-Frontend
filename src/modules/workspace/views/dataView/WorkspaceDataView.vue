<template>
    ...
    <div class="p-d-flex p-flex-row p-ai-center p-flex-wrap">
        <InputText v-model="searchWord" class="kn-material-input p-m-3 model-search" :style="mainDescriptor.style.filterInput" type="text" :placeholder="$t('common.search')" data-test="search-input" @input="searchItems" />
    ...
    <div class="kn-overflow">
        <DataTable v-if="!toggleCardDisplay" class="p-datatable-sm kn-table p-mx-2" :value="filteredDatasets" :loading="loading" data-key="objId" responsive-layout="stack" breakpoint="600px" data-test="datasets-table">
            <template #empty>
        ...
        <div v-if="toggleCardDisplay" class="p-grid p-m-2" data-test="card-container">
            ...
            <template v-else>
                <WorkspaceCard
                    ...
                    :view-type="'dataset'"
                    :document="dataset"
                    :is-avro-ready="isAvroReady(dataset.id)"
    ...
    <DetailSidebar
        ...
        :is-avro-ready="isAvroReady(selectedDataset?.id)"
        :dataset-categories="datasetCategories"
    ...
    <QBE v-if="qbeVisible" :visible="qbeVisible" :dataset="selectedQbeDataset" :source-dataset="selectedDataset" @close="closeQbe" />
    <DataPreparationMonitoringDialog v-model:visibility="showMonitoring" :dataset="selectedDataset" @close="showMonitoring = false" @save="updateDatasetWithNewCronExpression"></DataPreparationMonitoringDialog>
</template>
...
export default defineComponent({
    ...
    methods: {
        ...
        getDatasets(filter: string) {
            this.loading = true
            return this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/3.0/datasets/${filter}/`)
        },
        ...
        async getDatasetCategories() {
            this.loading = true
            return this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/domainsforfinaluser/ds-categories`).then((response: AxiosResponse<any>) => {
        ...
        async loadDatasetDrivers(dataset) {
            ...
            if (dataset.label && dataset.id && dataset.dsTypeCd !== 'Prepared') {
                const postPayload = { role: this.userRole || this.user.roles[0] }
                await this.$http
                    .post(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/3.0/datasets/${dataset.label}/filters`, postPayload)
                    .then((response: AxiosResponse<any>) => {
        ...
        getFormattedDrivers(parameters) {
            const formattedParameters = {} as any
        ...
        createMenuItems(clickedDocument: any) {
            ...
            tmp.push({ key: 8, label: this.$t('workspace.myData.cloneDataset'), icon: 'fas fa-clone', command: () => this.cloneDataset(clickedDocument), visible: this.canLoadData })
        ...
        createCreationMenuButtons(event: any) {
            ...
            this.creationMenuButtons.push({ key: '0', label: this.$t('managers.businessModelManager.uploadFile'), command: () => this.toggleDatasetDialog(), visible: true })
        },
        toggleDatasetDialog() {
        ...
        async updateDatasetWithNewCronExpression(newCron) {
            ...
            await this.$http.post(import.meta.env.VITE_KNOWAGE_DATA_PREPARATION_CONTEXT + '/api/1.0/process', newCron).then(
                () => {
