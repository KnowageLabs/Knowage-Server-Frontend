<template>
  <div class="kn-page kn-data-preparation">
    <KnCalculatedField v-model:visibility="showCFDialog" v-model:template="selectedTransformation" :fields="columns"
                       :descriptor="cfDescriptor" :prop-calc-field-functions="cfDescriptor.availableFunctions"
                       :read-only="readOnly" :valid="cfType !== ''" @save="saveCFDialog" @cancel="cancelCFDialog"
    ...
    <DataPreparationDialog v-model:transformation="selectedTransformation" v-model:col="col" :columns="columns"
                           :read-only="readOnly" @send-transformation="handleTransformation"
                           @update:readOnly="updateReadOnly"/>
    <DataPreparationSaveDialog v-model:visibility="showSaveDialog" :original-dataset="dataset"
                               :existing-dataset="existingDataset || ''" :config="dataset.config" :columns="columns"
                               :instance-id="instanceId" :process-id="processId" :prepared-ds-meta="preparedDsMeta"
                               @update:instanceId="updateInstanceId" @update:processId="updateprocessId"/>
    <Toolbar class="kn-toolbar kn-toolbar--primary p-m-0">
      <template #start> {{ $t('managers.workspaceManagement.dataPreparation.label') }}
        ({{ $t('managers.workspaceManagement.dataPreparation.originalDataset') }}: {{ dataset.name }})
      </template>
      <template #end>
        <Button v-tooltip.bottom="$t('common.refresh')" icon="pi pi-refresh"
                class="p-button-text p-button-rounded p-button-plain" :disabled="loading > 0"
                @click="refreshOriginalDataset"/>
        <Button v-tooltip.bottom="$t('common.save')" icon="pi pi-save"
                class="p-button-text p-button-rounded p-button-plain" :disabled="loading > 0" data-test="save-button"
                @click="saveDataset"/>
        <Button v-tooltip.bottom="$t('common.close')" icon="pi pi-times"
                class="p-button-text p-button-rounded p-button-plain" data-test="close-button"
                @click="closeTemplate()"/>
      </template
      >
    </Toolbar>
    <Toolbar class="kn-toolbar kn-toolbar--secondary p-m-0 toolbarCustomConfig">
      <template #start>
        <template v-for="(menu, index) in getMenuForToolbar()" :key="index">
          <Button v-if="menu !== 'divider'" v-tooltip.bottom="$t(menu.label)" :class="descriptor.css.buttonClassHeader"
                  :disabled="calculateDisabledProperty(menu)" @click="callFunction(menu)">
      ...
      <template #end>
        <div class="arrow-button-container">
          <Button icon="pi pi-arrow-left" :class="descriptor.css.buttonClassHeader" style="overflow: visible"
                  @click="toggleSidebarVisibility()"/>
          <Badge v-if="dataset.config && dataset.config.transformations && dataset.config.transformations.length > 0"
                 class="arrow-badge"
                 :value="dataset.config && dataset.config.transformations && dataset.config.transformations.length"></Badge>
        </div>
      </template>
    </Toolbar>
    ...
    <div class="kn-page-content p-grid p-m-0 managerDetail">
      <Sidebar v-model:visible="visibleRight" position="right" class="kn-data-preparation-sidenav">
        <div class="info-container">
          <div class="original-dataset">
            <i class="fa fa-database p-mr-2"></i><span>{{
              $t('managers.workspaceManagement.dataPreparation.originalDataset')
            }}</span
          >: {{ dataset.label }}
          </div>
        </div>
        ...
        >
          <template #option="slotProps">
            <div class="p-text-uppercase kn-list-item transformationSidebarElement">
              <div v-if="slotProps.option.type != 'calculatedField'">{{ slotProps.option.type }} -
                {{ slotProps.option.parameters[0].columns[0] }}
              </div>
              <div v-else>{{ slotProps.option.type }} - {{ slotProps.option.parameters[0].colName }}</div>
              <div>
                <Button v-if="slotProps.option.type != 'trim' && slotProps.option.type != 'drop'"
                        v-tooltip="$t('common.preview')" icon="fas fa-eye" :class="descriptor.css.buttonClassHeader"
                        @click="openTransformationDetail(slotProps.option)"/>
                <Button v-if="slotProps.index == 0" v-tooltip="$t('common.delete')" icon="p-jc-end pi pi-trash"
                        :class="descriptor.css.buttonClassHeader" :disabled="loading > 0"
      ...
      >
        ...
        <Column v-for="(col, colIndex) in columns" :key="colIndex" :field="col.header" :style="{ width: '200px' }">
          <template #header>
            <Button v-if="col.fieldType" :class="descriptor.css.buttonClassHeader"
                    @click="toggle($event, 'opType-' + colIndex)">
              <span v-if="descriptor.roles.filter((x) => x.code === col.fieldType)[0].icon.class"
                    :class="descriptor.roles.filter((x) => x.code === col.fieldType)[0].icon.class">{{
                  descriptor.roles.filter((x) => x.code === col.fieldType)[0].icon.name
                }}</span>
              <i v-else :class="descriptor.roles.filter((x) => x.code === col.fieldType)[0].icon"></i>
            </Button>
            <OverlayPanel :ref="'opType-' + colIndex" :popup="true">
                            <span class="p-float-label">
                                <Dropdown v-model="col.fieldType" :options="translateRoles()" option-label="label"
                                          option-value="code" class="kn-material-input"/>
                            </span>
            </OverlayPanel>
            <div class="aliasAndType p-ml-2">
              <input v-if="col.editing" v-model="col.fieldAlias" class="kn-input-text-sm" type="text"
                     @blur="changeAlias(col)" @keydown.enter="changeAlias(col)"/>
              <span v-else class="kn-clickable" @click="changeAlias(col)">{{ col.fieldAlias }}</span>
              <span class="kn-list-item-text-secondary kn-truncated roleType">{{
                  $t(removePrefixFromType(col.Type))
                }}</span>
            </div>
            <Button icon="pi pi-ellipsis-v" :class="descriptor.css.buttonClassHeader"
                    @click="toggle($event, 'trOpType-' + colIndex)"/>
            <Menu :ref="'trOpType-' + colIndex" :model="getTransformationsMenu(col)" :popup="true" data-test="menu">
              <template #item="{ item }">
                                <span
                                    :class="['p-menuitem-link', 'toolbarCustomConfig', descriptor.css.buttonClassHeader]"
                                    @click="callFunction(item, col)">
                                    <span v-if="item.icon.class" :class="item.icon.class"
                                          class="menu-icon">{{ item.icon.name }}</span>
          ...
          >
          <template #body="{ data }">
            <span v-if="col.Type.toLowerCase().includes('time')"> {{
                getFormattedDate(data[col.header], {
                  year: 'numeric',
                  month: '2-digit',
                  day: '2-digit',
                  hour: '2-digit',
            ...
            <span v-else-if="col.Type.toLowerCase().includes('date')"> {{
                getFormattedDate(data[col.header], {
                  year: 'numeric',
                  month: '2-digit',
                  day: '2-digit'
...
import DataPreparationSimpleDescriptor
  from '@/modules/workspace/dataPreparation/DataPreparationSimple/DataPreparationSimpleDescriptor.json'
import DataPreparationSplitDescriptor
  from '@/modules/workspace/dataPreparation/DataPreparationCustom/DataPreparationSplitDescriptor.json'
import calculatedFieldDescriptor
  from '@/modules/workspace/dataPreparation/DataPreparationCalculatedFieldDescriptor.json'
...
export default defineComponent({
  ...
  async created() {
    ...
    if (this.dataset) {
      ...
      await this.initTransformations()

      const url = new URL(window.location.origin)
      url.protocol = url.protocol.replace('http', 'ws')
      const uri = url.origin + import.meta.env.VITE_KNOWAGE_DATA_PREPARATION_CONTEXT + '/ws?' + import.meta.env.VITE_DEFAULT_AUTH_HEADER + '=' + sessionStorage.getItem('token')
