...
export default defineComponent({
    ...
    methods: {
        ...
        async getAllLayers() {
            await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/layers`).then((response: AxiosResponse<any>) => (this.documentData.allLayers = response.data.root))
        },

        async getSelectedDocument() {
            await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/2.0/documents/${this.$route.query.documentId}`).then(async (response: AxiosResponse<any>) => {
        ...
        async loadLayerColumns(layerId) {
            await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/layers/getFilter?id=${layerId}`).then((response: AxiosResponse<any>) => {
                this.documentData.layerJoinColumns = response.data
            })
        },
        async initializeDataset() {
            if (this.documentData.dataSetId) {
                await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/1.0/datasets/dataset/id/${this.documentData.dataSetId}`).then((response: AxiosResponse<any>) => {
        ...
        async loadDatasetColumns() {
            await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/1.0/datasets/${this.documentData.datasetLabel}/fields`).then((response: AxiosResponse<any>) => {
        ...
        async saveGisDocument(template) {
            if (this.$route.path.includes('edit')) {
                ...
                await this.$http.post(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/1.0/documents/saveGeoReportTemplate`, postData).then(async () => {
                ...
                await this.$http.post(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/2.0/saveDocument/`, postData).then(async () => {
                    this.saveDialogVisible = false
                    this.canOpenMap = true
        ...
        async routeToDocument() {
            await this.$http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/2.0/documentdetails/${this.$route.query.documentId}/templates`).then((response: AxiosResponse<any>) => {
