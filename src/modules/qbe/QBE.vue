<template>
    <Dialog class="full-screen-dialog" :visible="true" :modal="false" :closable="false" position="right" :base-z-index="1" :auto-z-index="true">
            ...
            <div v-if="showEntitiesLists && qbeLoaded" :style="qbeDescriptor.style.entitiesLists">
                ...
                <div class="p-d-flex p-flex-column kn-overflow-hidden" :class="{ 'derived-entities-toggle': showDerivedList }">
                    <Toolbar class="kn-toolbar kn-toolbar--secondary kn-flex-0">
                        ...
                        <template #end>
                            <Button v-if="showEntitiesLists" v-tooltip.top="$t('common.add')" icon="fas fa-plus-circle" class="p-button-text p-button-rounded p-button-plain" @click="createSubquery" />
                    ...
                    <div v-show="showDerivedList" class="kn-flex kn-overflow-hidden">
                        <ScrollPanel class="kn-height-full qbe-scroll-panel">
                            <SubqueryEntity :available-entities="mainQuery?.subqueries" @editSubquery="selectSubquery" @deleteSubquery="deleteSubquery" />
                        </ScrollPanel>
                    </div>
                </div>
            </div>
            <div v-if="qbe && qbeLoaded" class="qbe-detail-view p-m-1">
                <Toolbar class="kn-toolbar kn-toolbar--primary kn-width-full">
                    <template #start>
                        <Button v-if="showEntitiesLists" v-tooltip.bottom="$t('qbe.detailView.hideList')" icon="pi pi-chevron-left" class="p-button-text p-button-rounded p-button-plain" @click="toggleEntitiesLists" />
                        <Button v-else v-tooltip.bottom="$t('qbe.detailView.showList')" icon="pi pi-chevron-right" class="p-button-text p-button-rounded p-button-plain" @click="toggleEntitiesLists" />
        ...
        <QBEFilterDialog :id="uniqueID" :visible="filterDialogVisible" :filter-dialog-data="filterDialogData" :entities="entities?.entities" :prop-parameters="qbe?.pars" :prop-expression="selectedQuery?.expression" @close="filterDialogVisible = false" @save="onFiltersSave"></QBEFilterDialog>
        <QBESqlDialog :visible="sqlDialogVisible" :sql-data="sqlData" @close="sqlDialogVisible = false" />
        <QBERelationDialog :visible="relationDialogVisible" :prop-entity="relationEntity" @close="relationDialogVisible = false" />
        <QBEParamDialog v-if="paramDialogVisible" :visible="paramDialogVisible" :prop-dataset="qbe" @close="paramDialogVisible = false" @save="onParametersSave" />
        <QBEHavingDialog :visible="havingDialogVisible" :having-dialog-data="havingDialogData" :entities="selectedQuery?.fields" @close="havingDialogVisible = false" @save="onHavingsSave"> </QBEHavingDialog>
    ...
    methods: {
        ...
        hideSpatialFunct() {
            ...
            if (!this.selectedQuery.fields.some(isSpatial)) {
                const tempFunctions = deepcopy(this.calcFieldFunctions)
        ...
        addEntityToMainQuery(field, isCalcField?) {
            const queryModel = this.selectedQuery.fields
            ...
            if (!isCalcField) {
                const newField = createNewField(editQueryObj, field)
                const newMetadata = creatNewMetadataFromField(newField)
        ...
        deleteFieldFromQuery(fieldID) {
            const indexOfFieldToDelete = this.selectedQuery.fields.findIndex((field) => {
        ...
        deleteFieldMetadata(fieldID) {
            const indexOfFieldMetadataToDelete = this.qbeMetadata.findIndex((metadata) => {
        ...
        updateExistingCalculatedField(calculatedField) {
            const fieldToEditIndex = this.selectedQuery.fields.findIndex((field) => field.uniqueID == calculatedField.uniqueID)
        ...
        updateCalculatedFieldMetadata(calculatedField) {
            const metaToEditIndex = this.qbeMetadata.findIndex((meta) => meta.uniqueID == calculatedField.uniqueID)
        ...
        createMenuItems() {
            this.menuButtons = []
            const repetitionIcon = this.discardRepetitions ? 'fas fa-check' : 'fas fa-times'
        ...
        createSubquery() {
            const newSubquery = { id: 'q' + this.createQueryName(), name: 'subentity-q' + this.createQueryName(), fields: [], distinct: false, filters: [], calendar: {}, expression: {}, isNestedExpression: false, havings: [], graph: [], relationRoles: [], subqueries: [] } as any
        ...
        createQueryName() {
            let lastcount = 0
            const lastIndex = this.mainQuery.subqueries?.length - 1
            if (lastIndex != -1 && this.mainQuery.subqueries) {
                const lastQueryId = this.mainQuery.subqueries[lastIndex].id
