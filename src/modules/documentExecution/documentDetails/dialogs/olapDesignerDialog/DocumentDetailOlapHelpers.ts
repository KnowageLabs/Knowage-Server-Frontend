...
async function getSchema($http: any, schemaName: string) {
    let schema = null
    await $http.get(import.meta.env.VITE_KNOWAGE_CONTEXT + `/restful-services/2.0/mondrianSchemasResource/name=${schemaName}`).then((response: AxiosResponse<any>) => (schema = response.data))
    return schema
}
