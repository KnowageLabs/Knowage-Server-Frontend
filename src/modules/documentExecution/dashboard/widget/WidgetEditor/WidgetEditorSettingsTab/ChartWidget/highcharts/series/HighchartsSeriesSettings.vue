...
export default defineComponent({
    ...
    data() {
        return {
            ...
            availableColumnOptions: [] as string[],
            advancedVisible: {},
            getTranslatedLabel
        }
    },
    ...
    methods: {
        ...
        setEventListeners() {
            emitter.on('columnAdded', this.loadModel)
            emitter.on('columnRemoved', this.loadModel)
            emitter.on('chartTypeChanged', this.onChartTypeChanged)
        },
        removeEventListeners() {
            emitter.off('columnAdded', this.loadModel)
            emitter.off('columnRemoved', this.loadModel)
        ...
        loadSeriesOptions() {
            ...
            this.widgetModel.columns.forEach((column: IWidgetColumn) => {
                if (column.fieldType === 'MEASURE') this.availableColumnOptions.push(column.columnName)
            })
        },
        addFirstSeriesSetting() {
            if (!this.model) return
            this.seriesSettings = []
            if (this.availableColumnOptions.length >= 1) {
                const name = this.allSeriesOptionEnabled ? 'all' : this.availableColumnOptions[0]
        ...
        removeSerieFromAvailableOptions(seriesName: string) {
            const index = this.availableColumnOptions.findIndex((tempSerieName: string) => tempSerieName === seriesName)
        ...
        onSeriesAddedFromMultiselect(serieSetting: IHighchartsSeriesLabelsSetting) {
            serieSetting.names.forEach((serieName: string) => {
                const index = this.availableColumnOptions.findIndex((tempSerieName: string) => tempSerieName === serieName)
                if (index !== -1) this.availableColumnOptions.splice(index, 1)
            })
        },
        onSeriesRemovedFromMultiselect(intersection: string[]) {
            intersection.forEach((serieName: string) => this.availableColumnOptions.push(serieName))
        },
        addSerieSetting() {
        ...
        removeSerieSetting(index: number) {
            this.seriesSettings[index].names.forEach((serieName: string) => this.availableColumnOptions.push(serieName))
