  ...
  <div class="kn-height-full detail-page-container">
    ...
    <Toolbar
      v-if="
        ...
        !olapDesignerMode &&
        !managementOpened &&
        !dashboardGeneralSettingsOpened
      "
      class="kn-toolbar kn-toolbar--primary p-col-12"
    >
      <template #start>
        <DocumentExecutionBreadcrumb
          v-if="breadcrumbs.length > 1"
          :breadcrumbs="breadcrumbs"
          @breadcrumbClicked="onBreadcrumbClick"
        ></DocumentExecutionBreadcrumb>
        <span v-else>{{
      ...
      <template #end>
        <div class="p-d-flex p-jc-around">
          <Button
            v-if="
              mode == 'dashboard' &&
              propMode !== 'document-execution-cross-navigation-popup'
            "
          ...
          <Button
            v-if="
              mode !== 'dashboard' && canEditCockpit && documentMode === 'VIEW'
            "
          ...
          <Button
            v-if="
              mode !== 'dashboard' && canEditCockpit && documentMode === 'EDIT'
            "
          ...
          <Button
            v-if="mode !== 'dashboard'"
            v-tooltip.left="$t('common.onlineHelp')"
            icon="pi pi-book"
            class="p-button-text p-button-rounded p-button-plain p-mx-2"
            @click="openHelp"
          ></Button>
          ...
          ></Button>
          <Button
            v-if="propMode !== 'document-execution-cross-navigation-popup'"
            v-tooltip.left="$t('common.menu')"
          ...
          <Button
            ...
            class="p-button-sm"
            :label="$t('dashboard.widgetEditor.addWidget')"
            icon="pi pi-plus-circle"
            @click="addWidget"
          />
          <Button
            v-tooltip.left="$t('common.close')"
            icon="fa fa-times"
            class="p-button-text p-button-rounded p-button-plain p-mx-2"
            :class="{ 'dashboard-toolbar-icon': mode === 'dashboard' }"
            @click="closeDocumentConfirm"
          ></Button>
        </div>
      </template>
    </Toolbar>
    <ProgressBar
      v-if="loading || loadingCrossNavigationDocument"
      class="kn-progress-bar"
      mode="indeterminate"
    />
    <div
      ref="document-execution-view"
      class="p-d-flex p-flex-row document-execution-view myDivToPrint"
    >
      <div
        ...
        "
        @click="parameterSidebarVisible = false"
      ></div>
      <div v-show="showExecutedDocument || newDashboardMode" class="kn-flex">
        <Registry
          v-if="showExecutedDocument && mode === 'registry'"
          :id="urlData?.sbiExecutionId"
          :reload-trigger="reloadTrigger"
        ></Registry>
        <Dossier
          v-else-if="showExecutedDocument && mode === 'dossier'"
          :id="document.id"
          :reload-trigger="reloadTrigger"
        ...
        ></DashboardController>
        <div
          v-show="mode === 'dashboard' || newDashboardMode"
        ...
        >
          <template v-for="(item, index) in breadcrumbs" :key="index">
            <DashboardController
              :visible="
                ((filtersData && filtersData.isReadyForExecution) ||
                  newDashboardMode) &&
                !loading &&
                !schedulationsTableVisible &&
                (item.label === document.name ||
                  newDashboardMode ||
                  (crossNavigationDialogVisible &&
      ...
      <iframe
        ...
        v-show="
          ...
          !schedulationsTableVisible &&
          (item.label === document.name ||
            (crossNavigationContainerData && index === breadcrumbs.length - 1))
        "
        :key="index"
        ref="documentFrame"
      ...
      <DocumentExecutionSchedulationsTable
        v-if="schedulationsTableVisible"
        id="document-execution-schedulations-table"
        :prop-schedulations="schedulations"
        @deleteSchedulation="onDeleteSchedulation"
        @close="schedulationsTableVisible = false"
      ></DocumentExecutionSchedulationsTable>
      ...
      <DocumentExecutionRankDialog
        ...
        :prop-document-rank="documentRank"
      ...
      <DocumentExecutionNotesDialog
        ...
        :prop-document="document"
        @close="notesDialogVisible = false"
      ></DocumentExecutionNotesDialog>
      <DocumentExecutionMetadataDialog
        ...
        @saveMetadata="onMetadataSave"
      ></DocumentExecutionMetadataDialog>
      <DocumentExecutionMailDialog
        :visible="mailDialogVisible"
        @close="mailDialogVisible = false"
        @sendMail="onMailSave"
      ></DocumentExecutionMailDialog>
      <DocumentExecutionLinkDialog
        :visible="linkDialogVisible"
        :link-info="linkInfo"
        :embed-h-t-m-l="embedHTML"
        :prop-document="document"
        :parameters="linkParameters"
        @close="linkDialogVisible = false"
      ></DocumentExecutionLinkDialog>
      <DocumentExecutionSelectCrossNavigationDialog
        :visible="destinationSelectDialogVisible"
        :cross-navigation-documents="crossNavigationDocuments"
        @close="destinationSelectDialogVisible = false"
        @selected="onCrossNavigationSelected"
      ></DocumentExecutionSelectCrossNavigationDialog>
      <DocumentExecutionCNContainerDialog
        v-if="angularData && crossNavigationContainerData"
        :visible="crossNavigationContainerVisible"
        :data="crossNavigationContainerData"
        @close="onCrossNavigationContainerClose"
      ></DocumentExecutionCNContainerDialog>
      <Dialog
        class="p-fluid kn-dialog--toolbar--primary"
        :content-style="descriptor.popupDialog.style"
      ...
      >
        <DocumentExecution
          ...
          :tab-key="tabKey"
          :prop-cross-navigation-popup-dialog-document="
            crossNavigationPopupDialogDocument
          "
          @parametersChanged="$emit('parametersChanged', $event)"
...
import {
  ...
  iExporter,
  iSchedulation,
  ICrossNavigationBreadcrumb,
  ICrossNavigationParameter,
...
import {
  executeAngularCrossNavigation,
  loadCrossNavigation,
} from "./DocumentExecutionAngularCrossNavigationHelper";
import {
  getDocumentForCrossNavigation,
  getSelectedCrossNavigation,
  updateBreadcrumbForCrossNavigation,
} from "./DocumentExecutionCrossNavigationHelper";
...
window.execExternalCrossNavigation = function (
  outputParameters,
  otherOutputParameters,
  crossNavigationLabel
) {
  postMessage(
    {
      ...
      docLabel: null,
      otherOutputParameters: otherOutputParameters
        ? [otherOutputParameters]
        : [],
    },
    "*"
...
export default defineComponent({
  ...
  computed: {
    ...
    canEditCockpit(): boolean {
      ...
      return (
        (this.document.engine?.toLowerCase() === "knowagecockpitengine" ||
          this.document.engine?.toLowerCase() === "knowagedashboardengine") &&
        (this.user.functionalities?.includes(
          UserFunctionalitiesConstants.DOCUMENT_ADMIN_MANAGEMENT
        ) ||
          this.document.creationUser === this.user.userId)
      );
    },
    sessionRole(): string | null {
      if (!this.user) return null;
      return this.user.sessionRole !== this.$t("role.defaultRolePlaceholder")
    ...
    showExecutedDocument() {
      return (
        this.filtersData &&
        this.filtersData.isReadyForExecution &&
        !this.loading &&
        !this.schedulationsTableVisible
      );
    },
  },
  watch: {
    async menuItemClickedTrigger() {
      ...
      if (!test) return;
      const label = this.selectedMenuItem.to.substring(
        this.selectedMenuItem.to.lastIndexOf("/") + 1
      );
  ...
  async created() {
    ...
    if (this.propCrossNavigationPopupDialogDocument) {
      ...
      if (
        this.propMode !== "document-execution" &&
        !this.$route.path.includes("olap-designer") &&
        this.$route.name !== "document-execution" &&
        this.$route.name !== "document-execution-embed" &&
        this.$route.name !== "document-execution-workspace"
      )
        return;
  ...
  methods: {
    ...
    export(type: string) {
      if (this.document.typeCode === "OLAP") {
        ...
        window.open(
          this.urlData?.url + "&outputType=" + type,
          "name",
          "resizable=1,height=750,width=1000"
        );
      } else {
        const tempIndex = this.breadcrumbs.findIndex(
          (el: any) => el.label === this.document.name
        );
        let tempFrame = window.frames[tempIndex];
        while (tempFrame && tempFrame.name !== "documentFrame" + tempIndex)
          tempFrame = tempFrame[0].frames;

        tempFrame.postMessage(
          { type: "export", format: type.toLowerCase() },
          "*"
        );
      }
    },
    exportOlap(type: string) {
      const url =
        type === "PDF"
          ? `/knowagewhatifengine/restful-services/1.0/model/export/pdf?SBI_EXECUTION_ID=${this.sbiExecutionId}`
          : `/knowagewhatifengine/restful-services/1.0/model/export/excel?SBI_EXECUTION_ID=${this.sbiExecutionId}`;
      window.open(url);
    },
    openMailDialog() {
    ...
    async openMetadata() {
      ...
      await this.$http
        .get(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `1.0/documentexecutionee/${this.document.id}/documentMetadata`
        )
        .then(
          (response: AxiosResponse<any>) => (this.metadata = response.data)
        );
    ...
    async showScheduledExecutions() {
      ...
      await this.$http
        .get(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `1.0/documentsnapshot/getSnapshots?id=${this.document.id}`
        )
        .then((response: AxiosResponse<any>) =>
          response.data?.schedulers.forEach((el: any) =>
            this.schedulations.push({ ...el, urlPath: response.data.urlPath })
          )
        );
      this.loading = false;
    },
    async copyLink(embedHTML: boolean) {
      ...
      await this.$http
        .post(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `1.0/documentexecution/canHavePublicExecutionUrl`,
    ...
    closeDocumentConfirm() {
      if (this.documentMode === "EDIT") {
        this.$confirm.require({
          message: this.$t(
            "documentExecution.main.closeDocumentConfirmMessage"
          ),
    ...
    closeDocument() {
      if (this.propMode !== "document-execution-cross-navigation-popup") {
        this.$router.push(
          this.$route.path.includes("workspace")
            ? "/workspace"
            : "/document-browser"
        );
        this.breadcrumbs = [];
    ...
    async loadPage(
      ...
      documentLabel: string | null = null,
      crossNavigationPopupMode = false
    ) {
      this.loading = crossNavigationPopupMode ? false : true;
      this.filtersData = await loadFilters(
        initialLoading,
        this.filtersData,
        this.document,
        this.breadcrumbs,
    ...
    updateMode() {
      if (this.document.typeCode === "DATAMART") this.mode = "registry";
        ...
        (this.document.typeCode === "DOCUMENT_COMPOSITE" &&
          this.$route.path.includes("dashboard")) ||
        this.document.typeCode === "DASHBOARD"
      )
        this.mode = "dashboard";
      else this.mode = "iframe";
    },
    async loadDocument() {
      await this.$http
        .get(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `2.0/documents/${this.document?.label}`
        )
        .then(
          (response: AxiosResponse<any>) => (this.document = response.data)
        );
    ...
    async loadURL(
      ...
      const postData = {
        label: this.document.label,
        role: this.userRole,
        parameters: olapParameters
          ? olapParameters
          : this.getFormattedParameters(),
    ...
    async loadExporters() {
      ...
      if (!this.urlData || !this.urlData.engineLabel) return;
      const engineLabel =
        "knowagedashboardengine" === this.urlData.engineLabel
          ? "knowagecockpitengine"
          : this.urlData.engineLabel;
      await this.$http
        .get(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `2.0/exporters/${engineLabel}`
        )
        .then(
    ...
    async sendForm(
      ...
      const documentUrl =
        this.urlData?.url + "&timereloadurl=" + new Date().getTime();
      const postObject = {
        params: { document: null } as any,
        url: documentUrl.split("?")[0],
      };
      if (this.$route.query.documentMode === "edit" && !this.newCockpitCreated)
        this.documentMode = "EDIT";
      ...
      for (const i in paramsFromUrl) {
        if (typeof paramsFromUrl !== "function")
          postObject.params[paramsFromUrl[i].split("=")[0]] =
            paramsFromUrl[i].split("=")[1];
      }

      let postForm = document.getElementById(
        "postForm_" + postObject.params.document
      ) as any;
      ...
      const iframeName = crossNavigationPopupMode
        ? "documentFramePopup"
        : "documentFrame";
      postForm.target =
        tempIndex !== -1 ? iframeName + tempIndex : documentLabel;
      ...
      for (const k in postObject.params) {
        const inputElement = document.getElementById(
          "postForm_" + postObject.params.document + k
        ) as any;
        if (inputElement) {
          inputElement.value = decodeURIComponent(postObject.params[k]);
          inputElement.value = inputElement.value.replace(/\+/g, " ");
          this.hiddenFormData.set(
            k,
            decodeURIComponent(postObject.params[k]).replace(/\+/g, " ")
          );
        } else {
      ...
      for (let i = postForm.elements.length - 1; i >= 0; i--) {
        const postFormElement = postForm.elements[i].id.replace(
          "postForm_" + postObject.params.document,
          ""
        );
      ...
      this.document.typeCode === "DATAMART" ||
      this.document.typeCode === "DOSSIER" ||
      this.document.typeCode === "OLAP" ||
      (this.document.typeCode === "DOCUMENT_COMPOSITE" &&
        this.mode === "dashboard")
      ...
      if (index !== -1)
        this.breadcrumbs[index].hiddenFormData = this.hiddenFormData;
    },
    ...
    async onExecute() {
      ...
      if (!this.exporters || this.exporters.length === 0) {
        await this.loadExporters();
        const index = this.toolbarMenuItems.findIndex(
          (item: any) => item.label === this.$t("common.export")
        );
        index === -1
    ...
    async onExportCSV() {
      ...
      await this.$http
        .post(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH + `2.0/export/cockpitData`,
          postData
        )
        .then(() =>
          this.setInfo({
      ...
      Object.keys(this.filtersData.filterStatus).forEach((key: any) => {
        ...
        if (parameter.parameterValue) {
          if (
            ...
            parameter.parameterValue[0].value
          ) {
            parameters[parameter.urlName] = this.getFormattedDate(
              parameter.parameterValue[0].value
            );
            parameters[parameter.urlName + "_field_visible_description"] =
              this.getFormattedDate(parameter.parameterValue[0].value, true);
          } else if (parameter.valueSelection === "man_in") {
            if (!parameter.parameterValue[0])
              parameter.parameterValue[0] = { value: "", description: "" };
            parameters[parameter.urlName] =
              parameter.type === "NUM" && parameter.parameterValue[0].value
                ? +parameter.parameterValue[0].value
                : parameter.parameterValue[0].value;
            parameters[parameter.urlName + "_field_visible_description"] =
              parameter.type === "NUM" &&
            ...
            parameter.selectionType === "TREE" ||
            parameter.selectionType === "LOOKUP" ||
            parameter.multivalue
          ) {
            parameters[parameter.urlName] = parameter.parameterValue.map(
              (el: any) => el.value
            );
            let tempString = "";
            for (let i = 0; i < parameter.parameterValue.length; i++) {
              ...
              tempString +=
                i === parameter.parameterValue.length - 1 ? "" : ";";
            }
            parameters[parameter.urlName + "_field_visible_description"] =
              tempString;
          } else {
            parameters[parameter.urlName] = parameter.parameterValue[0]
              ? parameter.parameterValue[0].value
              : "";
            parameters[parameter.urlName + "_field_visible_description"] =
              parameter.parameterValue[0]
      ...
      Object.keys(this.filtersData.filterStatus).forEach((key: any) => {
        ...
        if (parameter.parameterValue) {
          if (parameter.type === "DATE") {
            ...
            parameter.valueSelection === "man_in" &&
            !parameter.multivalue
          ) {
            parameters[parameter.urlName] =
              parameter.type === "NUM" && parameter.parameterValue[0].value
    ...
    async getRank() {
      ...
      await this.$http
        .post(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH + `documentrating/getvote`,
          { obj: this.document.id }
        )
        .then(
          (response: AxiosResponse<any>) => (this.documentRank = response.data)
        )
        .catch((error: any) =>
          this.setError({ title: this.$t("common.error.generic"), msg: error })
        );
      this.loading = false;
    },
    async onSaveRank(newRank: any) {
      if (newRank) {
        ...
        await this.$http
          .post(
            import.meta.env.VITE_RESTFUL_SERVICES_PATH + `documentrating/vote`,
            { rating: newRank, obj: this.document.id }
          )
          .then(() =>
            this.setInfo({
              title: this.$t("common.toast.updateTitle"),
    ...
    async onMetadataSave(metadata: any) {
      ...
      await this.$http
        .post(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `1.0/documentexecutionee/saveDocumentMetadata`,
          { id: this.document.id, jsonMeta: jsonMeta }
        )
        .then(() => {
          this.setInfo({
    ...
    async onMailSave(mail: any) {
      ...
      await this.$http
        .post(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `1.0/documentexecutionmail/sendMail`,
          postData
        )
        .then(() => {
          this.setInfo({
    ...
    async onDeleteSchedulation(schedulation: any) {
      ...
      await this.$http
        .post(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `1.0/documentsnapshot/deleteSnapshot`,
          { SNAPSHOT: "" + schedulation.id }
        )
        .then(async () => {
    ...
    removeSchedulation(schedulation: iSchedulation) {
      const index = this.schedulations.findIndex(
        (el: any) => el.id === schedulation.id
      );
    ...
    getFormattedDate(date: any, useDefaultFormat?: boolean) {
      ...
      return luxonFormatDate(
        date,
        format,
        useDefaultFormat
          ? undefined
          : this.configurations["SPAGOBI.DATE-FORMAT-SERVER.format"]
      );
    },
    async onBreadcrumbClick(item: any) {
      ...
      this.parameterSidebarVisible =
        !this.filtersData || this.filtersData.isReadyForExecution === false;
      this.updateMode();
    },
    ...
    setNavigationParametersFromCurrentFilters(
      formatedParams: any,
      navigationParams: any
    ) {
      const navigationParamsKeys = navigationParams
        ? Object.keys(navigationParams)
        : [];
      const formattedParameters = this.getFormattedParameters();
      const formattedParametersKeys = formattedParameters
        ? Object.keys(formattedParameters)
        : [];
      if (
        navigationParamsKeys.length > 0 &&
        formattedParametersKeys.length > 0
      ) {
        for (let i = 0; i < navigationParamsKeys.length; i++) {
          const index = formattedParametersKeys.findIndex(
            (key: string) =>
              key === navigationParams[navigationParamsKeys[i]].value.label &&
              navigationParams[navigationParamsKeys[i]].value.isInput
          );
          if (index !== -1) {
            formatedParams[navigationParamsKeys[i]] =
              formattedParameters[formattedParametersKeys[index]];
            formatedParams[
              navigationParamsKeys[i] + "_field_visible_description"
            ] = formattedParameters[
              formattedParametersKeys[index] + "_field_visible_description"
            ]
              ? formattedParameters[
                  formattedParametersKeys[index] + "_field_visible_description"
                ]
    ...
    async executeOLAPCrossNavigation(crossNavigationParams: any) {
      ...
      await this.$http
        .get(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `1.0/crossNavigation/${this.document.label}/loadCrossNavigationByDocument`
        )
        .then((response: AxiosResponse<any>) => (temp = response.data));
      this.loading = false;
      ...
      this.document = {
        ...temp[0].document,
        navigationParams: this.formatOLAPNavigationParams(
          crossNavigationParams,
          temp[0].navigationParams
        ),
      };
      const index = this.breadcrumbs.findIndex(
        (el: any) => el.label === this.document.name
    ...
    formatOLAPNavigationParams(
      crossNavigationParams: any,
      navigationParams: any
    ) {
      ...
      Object.keys(navigationParams).forEach((key: string) => {
        const index = crossNavigationParamKeys.findIndex(
          (el: string) => el === navigationParams[key].value.label
        );
        if (index !== -1)
          formattedParams[key] =
            crossNavigationParams[crossNavigationParamKeys[index]];
      });
    ...
    isOrganizerEnabled() {
      return (
        ...
        this.user.functionalities.includes(
          UserFunctionalitiesConstants.SAVE_INTO_FOLDER_FUNCTIONALITY
        )
      );
    },
    async addToWorkspace() {
      ...
      await this.$http
        .post(
          import.meta.env.VITE_RESTFUL_SERVICES_PATH +
            `2.0/organizer/documents/${this.document.id}`,
          {},
        ...
        .catch((error) =>
          this.setError({
            ...
            msg:
              error.message ===
              "sbi.workspace.organizer.document.addtoorganizer.error.duplicateentry"
                ? this.$t("documentExecution.main.addToWorkspaceError")
                : error.message,
          })
        );
      this.loading = false;
    ...
    async loadUserConfig() {
      this.sessionEnabled =
        this.configurations["SPAGOBI.SESSION_PARAMETERS_MANAGER.enabled"] ===
        "false"
          ? false
          : true;
      this.dateFormat =
        this.configurations["SPAGOBI.DATE-FORMAT-SERVER.format"] === "%Y-%m-%d"
          ? "dd/MM/yyyy"
          : this.configurations["SPAGOBI.DATE-FORMAT-SERVER.format"];
    },
    saveParametersInSession() {
      ...
      tempFilters.filterStatus?.forEach((filter: any) =>
        [
          "dataDependsOnParameters",
          "dataDependentParameters",
          "dependsOnParameters",
          "dependentParameters",
          "lovDependsOnParameters",
          "lovDependentParameters",
        ].forEach((field: string) => delete filter[field])
      );
    ...
    setEventListeners() {
      ...
      emitter.on(
        "dashboardGeneralSettingsClosed",
        this.onDashboardGeneralSettingsClosed
      );
    },
    ...
    async onExecuteCrossNavigation(payload: {
      documentCrossNavigationOutputParameters: ICrossNavigationParameter[];
      crossNavigationName: string | undefined;
      crossNavigations: IDashboardCrossNavigation[];
    }) {
      ...
      const selectedCrossNavigation = getSelectedCrossNavigation(
        payload.crossNavigationName,
        payload.crossNavigations
      );
      ...
      this.executeCrossNavigation(
        selectedCrossNavigation,
        payload.documentCrossNavigationOutputParameters
      );
    },
    async getDocumentAfterCrossNavigationIsSelected(
      crossNavigation: IDashboardCrossNavigation
    ) {
      const documentCrossNavigationParameters = this.crossNavigationPayload
        ? this.crossNavigationPayload.documentCrossNavigationOutputParameters
        : [];
      this.executeCrossNavigation(
        crossNavigation,
        documentCrossNavigationParameters
      );
    },
    async executeCrossNavigation(
      crossNavigation: IDashboardCrossNavigation,
      documentCrossNavigationParameters: ICrossNavigationParameter[]
    ) {
      if (crossNavigation.crossType === 2) {
        const tempDocument = getDocumentForCrossNavigation(
          documentCrossNavigationParameters,
          this.filtersData,
          crossNavigation
        );
        ...
        this.crossNavigationPopupDialogDocument = getDocumentForCrossNavigation(
          documentCrossNavigationParameters,
          this.filtersData,
          crossNavigation
        );
        ...
        this.document = getDocumentForCrossNavigation(
          documentCrossNavigationParameters,
          this.filtersData,
          crossNavigation
        );
    ...
    openCrossNavigationInNewWindow(
      tempDocument: any,
      crossNavigation: IDashboardCrossNavigation
    ) {
      const parameters = encodeURI(
        JSON.stringify(tempDocument.formattedCrossNavigationParameters)
      );
      const url =
        import.meta.env.VITE_HOST_URL +
        `/knowage-vue/document-browser/dashboard/${this.document.label}?role=${this.userRole}&crossNavigationParameters=${parameters}`;
      ...
      window.open(
        url,
        "_blank",
        `toolbar=0,status=0,menubar=0,width=${popupOptions.width},height=${popupOptions.height}`
      );
    },
