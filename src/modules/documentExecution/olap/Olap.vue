...
export default defineComponent({
    ...
    methods: {
        ...
        async loadOlapButtons() {
            this.loading = true
            await this.$http
                .get(import.meta.env.VITE_KNOWAGEWHATIF_CONTEXT + `/restful-services/1.0/buttons`)
                .then(async (response: AxiosResponse<any>) => {
        ...
        async loadVersions() {
            ...
            await this.$http
                .get(import.meta.env.VITE_KNOWAGEWHATIF_CONTEXT + `/restful-services/1.0/version?SBI_EXECUTION_ID=${this.id}`)
        ...
        async sortOlap(event: any) {
            ...
            await this.$http
                .post(import.meta.env.VITE_KNOWAGEWHATIF_CONTEXT + `/restful-services/1.0/member/sort/?SBI_EXECUTION_ID=${this.id}`, postData)
                .then((response: AxiosResponse<any>) => (this.olap = response.data))
                .catch(() => {})
            this.formatOlapTable()
            this.loading = false
